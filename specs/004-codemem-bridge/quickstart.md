...
| T060 | after run 7 on the live map (the Architect's request, 2026-09-16, after the backup): `dotnet test CodeMem.sln --filter "FullyQualifiedName~B08_"` armed | first run 4 passed / 3 failed — (5) references(3023) 1 not 0, (6) MemOS `current` not behind/dirty, (7) presented 15515 not 15570: run 7's own effects, diagnosed read-only and re-pinned in the facts (B08 header, RED 2026-09-16 T060); then 7 passed / 0 failed; the unarmed suite is unaffected (B08 skips). These edits, the T060 records and this row follow the two commits and are uncommitted |
...
| 7. live MemOS run 7 | current; store hash unchanged | **run 2026-09-16 at the Architect's request (T060)**, backup `C:\_DB\codemem.sqlite.bak-2026-09-16` taken first (64 253 952 bytes, hash `ab23953e…` equal to the live map's). `extract.enabled` set true in the config beside the Release executable; `CodeMem.Bridge.exe extract --solution-key MemOS` → exit 0, origin `manual`, launched, child exit 0, **run 7**, 24 596 ms, line `solution=MemOS run_id=7 observed=15792 matched=14650 reactivated=0 new=1142 retired=63 registry_before=14713 notes_orphaned=0 candidates=0 unaccounted_observed=0 unaccounted_registry=0 digest=051faf7d… sha=cbeb6615…` (the counts balance: 14650 + 1142 = 15792; 14713 − 14650 − 63 = 0; the digest and counts equal the copy run's, T058). `map_status` before: MemOS `behind` — "HEAD cbeb6615 is 11 commits past the recorded 806f5f3f; the tree is clean."; after: MemOS `current` — "HEAD equals the recorded cbeb6615 and the tree is clean."; `solutions` MemOS run 6 → 7. `memos.sqlite` hash `c66e200d…` before and after (it had moved from `3b3f9aa2…` since the T054 run by a hand outside this feature: the PM store is live); MemOS `git status --porcelain` empty before and after; no journal; `extract.enabled` set back to false. The live map's hash is now `858c3ab5…`. Finding 152646 closed with live evidence. B08 armed afterwards: (5), (6), (7) red as the rule says a changed live number must be, each diagnosed read-only and re-pinned — references(3023) is 1 (a `uses` in `CodeMemRepoRootMatcherTests.vb`, a test added after run 6; type_usages 25 with 19 constructor calls, 21 from outside), MemOS `current` at `cbeb6615` with behindBy 0, 277 twin pairs (15792 active rows → 15515 declarations; the eleven commits added 55 pairs, every group still a pair); then 7 of 7, `map_status` 200 ms |
