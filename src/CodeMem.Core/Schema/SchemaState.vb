...
''' <summary>
''' The five states a file at <c>--db</c> can be in when the extractor holds the write lock. Detection is one method,
''' <c>MapDatabase.InspectSchema</c> (Article XII); the action for each state is taken by <c>ExtractionRun</c>.
''' </summary>
Public Enum SchemaState
    ''' <summary>The file is absent, 0 bytes, or a SQLite file whose sqlite_master has no user table (FR-105): create version 1, insert the identity row, migrate to 2.</summary>
    Fresh
    ''' <summary>map_identity.schema_version = 1: migrate to 2 in place (FR-112).</summary>
    Version1
